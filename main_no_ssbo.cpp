...
layout(location=0) in vec2 text_offset;
...
void main() {
  ...
  frag_color = mix(frag_color, vec4(1.0f), current_pixel);
    ...
    _sg_shader_desc.uniform_blocks[0] = {
        .stage = SG_SHADERSTAGE_VERTEX,
        .size = sizeof(vs_params),
        .glsl_uniforms = {
            { .type = SG_UNIFORMTYPE_FLOAT2, .glsl_name = "start_pos" },
            { .type = SG_UNIFORMTYPE_FLOAT2, .glsl_name = "screen_resolution" },
            { .type = SG_UNIFORMTYPE_FLOAT,  .glsl_name = "font_size" },
        },
    };
    ...
    _sg_pipeline_desc.layout.buffers[0] = {.stride = sizeof(sb_texts), .step_func = SG_VERTEXSTEP_PER_INSTANCE};
    _sg_pipeline_desc.layout.attrs[0] = { .buffer_index = 0, .offset = 0, .format = SG_VERTEXFORMAT_FLOAT2};
